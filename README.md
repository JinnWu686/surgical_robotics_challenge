...
This project is a fork of the Surgical Robotics Challenge. The main changes introduced in this version include:

- **Camera Enhancement:** Modified `world_stereo.yaml` to incorporate a back-view camera for enhanced perspective.
- **Light Source Control:** Added `world_stereo_varylight.yaml` to allow control over the light source position, enabling varied lighting conditions.
- **Wrist Cameras:** Added `world_stereo_wrist.yaml` to support two wrist-mounted cameras on the PSM arms, improving visibility during procedures.
